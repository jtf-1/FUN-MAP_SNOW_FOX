--///////////////////////////////////////////////////////////
-- Name: Operation Snowfox MkIII Escalation - Setsuka Module
...
env.info(" Setsuka's Sword Loading", false)
...
function SEF_SetukasSword()
  ...
  --////ARCO NOT ALIVE SCRUB DATA
	...
	--////IRIAF C-130
	if ( GROUP:FindByName(IRIAFC130GROUPNAME) ~= nil and GROUP:FindByName(IRIAFC130GROUPNAME):IsAlive() ) then				
		
		local SpawnUnits = GROUP:FindByName(IRIAFC130GROUPNAME):GetUnits()
		...
		for UnitID, UnitData in pairs( SpawnUnits ) do
			...
			--////Existing Timestamp
			if ( IRIAFC130_DATA[UnitID].Vec2 ~= nil and IRIAFC130_DATA[UnitID].TimeStamp ~= nil ) then
				--////On Ground And Stationary
				if SpawnUnit:InAir() == false and SpawnUnit:GetVelocityKMH() < 1 then
					local NewVec2 = SpawnUnit:GetVec2()
					if IRIAFC130_DATA[UnitID].Vec2.x == NewVec2.x and IRIAFC130_DATA[UnitID].Vec2.y == NewVec2.y then
						if IRIAFC130_DATA[UnitID].TimeStamp + CleanupTime < timer.getTime() then						
							env.info("IRIAFC130 Monitor - Deleting: "..SpawnUnitName.." Due To Inactivity", false)
							IRIAFC130_DATA[UnitID].Vec2 = nil
							IRIAFC130_DATA[UnitID].TimeStamp = nil
							Unit.getByName(SpawnUnit:GetName()):destroy()						
						end
					else
						IRIAFC130_DATA[UnitID].TimeStamp = timer.getTime()
						IRIAFC130_DATA[UnitID].Vec2 = SpawnUnit:GetVec2()
					end    		
				--////In Air Or On Ground And Moving On Taxi/Takeoff Etc
				else
					IRIAFC130_DATA[UnitID].Vec2 = nil
	...
	--////IRIAFC130 NOT ALIVE SCRUB DATA
	else
	...
	--////IRIAF SU-24M
	if ( GROUP:FindByName(IRIAFSU24MGROUPNAME) ~= nil and GROUP:FindByName(IRIAFSU24MGROUPNAME):IsAlive() ) then				
		
		local SpawnUnits = GROUP:FindByName(IRIAFSU24MGROUPNAME):GetUnits()
		...
		for UnitID, UnitData in pairs( SpawnUnits ) do
			...
			--////Existing Timestamp
			if ( IRIAFSU24M_DATA[UnitID].Vec2 ~= nil and IRIAFSU24M_DATA[UnitID].TimeStamp ~= nil ) then
				--////On Ground And Stationary
				if SpawnUnit:InAir() == false and SpawnUnit:GetVelocityKMH() < 1 then
					local NewVec2 = SpawnUnit:GetVec2()
					if IRIAFSU24M_DATA[UnitID].Vec2.x == NewVec2.x and IRIAFSU24M_DATA[UnitID].Vec2.y == NewVec2.y then
						if IRIAFSU24M_DATA[UnitID].TimeStamp + CleanupTime < timer.getTime() then						
							env.info("IRIAFSU24M Monitor - Deleting: "..SpawnUnitName.." Due To Inactivity", false)
							IRIAFSU24M_DATA[UnitID].Vec2 = nil
							IRIAFSU24M_DATA[UnitID].TimeStamp = nil
							Unit.getByName(SpawnUnit:GetName()):destroy()						
						end
					else
						IRIAFSU24M_DATA[UnitID].TimeStamp = timer.getTime()
						IRIAFSU24M_DATA[UnitID].Vec2 = SpawnUnit:GetVec2()
					end    		
				--////In Air Or On Ground And Moving On Taxi/Takeoff Etc
				else
					IRIAFSU24M_DATA[UnitID].Vec2 = nil
	...
	--////IRIAFSU24M NOT ALIVE SCRUB DATA
	else
...
env.info("Setsuka's Sword Initialized", false)
